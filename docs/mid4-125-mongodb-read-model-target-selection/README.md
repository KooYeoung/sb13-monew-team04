...
- MID4-96 설계에서는 activity와 snapshot의 mutable 상태를 payload의 과거 값이 아니라 worker가 batch 조회한 RDB 현재 상태로 수렴시키는 기준을 구체화했다.
...
- event ID, event type, aggregate ID, actor user ID, occurredAt은 Outbox 공통 컬럼이 담당한다.
