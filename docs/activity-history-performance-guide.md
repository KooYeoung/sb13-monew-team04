...
- MID4-136에서 Outbox 기본 테이블과 JPA 저장 모델을 준비했지만 도메인 쓰기 연동과 worker는 아직 없으므로 요청 흐름은 바뀌지 않았다.
...
- MongoDB 환경·인덱스와 Outbox 기본 저장 구조는 준비됐으므로 적용 조건이 충족되면 event sequence 직렬화, 도메인 이벤트 연동, document, projection writer, Outbox worker와 조회 전환을 순서대로 구현한다.
